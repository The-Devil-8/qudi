...
from socket import socket, AF_INET, SOCK_STREAM #John added
...
class MicrowaveSRSSG(Base, MicrowaveInterface):
    ...
    mw_source_srssg:
        module.Class: 'microwave.mw_source_srssg_copy.MicrowaveSRSSG'
        awg_ip_address: '10.107.54.148' #John added
        awg_port: 5025 # the port number as integer  #John added

        #Deleted gpib_address: 'GPIB0::12::INSTR'
        #Deleted gpib_timeout: 10

    """
    ip_address = ConfigOption('awg_ip_address', missing='error') #John added
    port = ConfigOption('awg_port', missing='error') #John added

    #DELETE_gpib_address = ConfigOption('gpib_address', missing='error')
    #DELETE_gpib_timeout = ConfigOption('gpib_timeout', 10, missing='warn')
    ...
    def on_activate(self):
        ...
        self.soc = socket(AF_INET, SOCK_STREAM) #John added

        self.soc.connect((self.ip_address, self.port)) #John added
        '''DELETED
        ...
        print("on activate", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
    def on_deactivate(self):
        ...
        self.soc.close()
        print("on deactivate", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return
    ...
    def cw_on(self):
        ...
        self.on()
        print("cw_on", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return 0
    ...
    def set_list(self, frequency=None, power=None):
        ...
        #DELETED self._gpib_connection.write('LSTD')

        self._write('LSTD')  #self.soc.send(bytes('LSTD', 'UTF-8'))  is wrong for some reason!!! Ig its the \n!!!
        ...
        print("set_list", frequency, power, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
    ...
    def reset_listpos(self):
        ...
        self._write('LSTR')
        print("reset_listpos", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return 0
    ...
    def set_sweep(self, start, stop, step, power):
        ...
        self.log.error('This was never tested!')
        print("set_sweep",start, stop, step, power, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
    ...
    def reset_sweeppos(self):
        ...
        self._internal_mode = 'sweep'
        self.log.error('This was never tested!')
        print("reset_sweeppos", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
    ...
    def set_ext_trigger(self, pol, timing):
        ...
        print("set_ext_trigger", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return pol, timing
    ...
    def trigger(self):
        ...
        print("trigger", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return
    ...
    def _ask(self, question):
        ...
        """
        #DELETED return self._gpib_connection.query(question)
        ...
        try:
            message = self.soc.recv(int(4096))  # receive an answer
        ...
        print("_ask",question, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return message
    ...
    def _write(self, command, wait=True):
        ...
        statuscode = self.soc.send(command)
        if wait:
            self.soc.send(bytes('*WAI\n', 'UTF-8'))  #The \n again
        print("write", command, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return statuscode


        ''' Alternatively
        sent = self.soc.send(command.encode())
        if sent == 0:
    ...
    def on(self):
        ...
        while not is_running:
            time.sleep(0.1)
            dummy, is_running = self.get_status()
        print("on", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return 0
    ...
    def set_power(self, power=0.):
        ...
        print("set_power",power, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return 0
    ...
    def set_frequency(self, freq=0.):
        ...
        print("set_frequency",freq, self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))
        return 0
    ...
    def reset_device(self):
        ...
        print("reset_device", self.soc.send(bytes('SYST:ERR?\n', 'UTF-8')))

    def error(self):
        try:
            self.soc.send(b'LERR?\n')
